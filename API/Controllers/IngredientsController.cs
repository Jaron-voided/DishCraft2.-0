...
public class IngredientsController : BaseApiController
{
    ...
    public async Task<ActionResult<IngredientDto>> CreateIngredient(IngredientDto ingredientDto)
    {
        ...
        Console.WriteLine($"  - Quantity: {ingredientDto?.Quantity ?? 0}");
        Console.WriteLine($"  - Calories: {ingredientDto?.Calories ?? 0}");
        Console.WriteLine($"  - Carbs: {ingredientDto?.Carbs ?? 0}");
        Console.WriteLine($"  - Fat: {ingredientDto?.Fat ?? 0}");
        Console.WriteLine($"  - Protein: {ingredientDto?.Protein ?? 0}");
